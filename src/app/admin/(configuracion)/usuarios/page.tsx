...
export default function UsuariosPage() {
  ...
  >([
    { campo: 'nombres', nombre: 'Nombres y Apellidos', ordenar: true },
    { campo: 'nroDocumento', nombre: 'Nro. Documento', ordenar: true },
    { campo: 'usuario', nombre: 'Usuario', ordenar: true },
    { campo: 'correoElectronico', nombre: 'Correo' },
    { campo: 'rol', nombre: 'Roles asignados', ordenar: false },
  ...
  const contenidoTabla: Array<Array<ReactNode>> = usuariosData.map(
    (usuarioData, indexUsuario) => [
      ...
      >
        <Image src={'/avatar.png'} alt="Foto" width={40} height={30} />
      ...
      <Box key={`${usuarioData.id}-${indexUsuario}-correo`}>
        {usuarioData.correoElectronico && (
          <Chip
            size={'small'}
            color={'primary'}
            label={usuarioData.correoElectronico}
            variant="outlined"
          />
        )}
        {!usuarioData.correoElectronico && (
      ...
      <Grid key={`${usuarioData.id}-${indexUsuario}-roles`}>
        ...
        >
          <MenuItem value="" hidden>
            ver ({usuarioData.usuarioRol.length}){' '}
            {`${usuarioData.usuarioRol.length > 1 ? 'roles' : 'rol'}`}
          </MenuItem>
          {usuarioData.usuarioRol.map((itemUsuarioRol, indexUsuarioRol) => (
            <MenuItem
              key={`usuario-rol-${indexUsuarioRol}`}
              value={itemUsuarioRol.rol.id}
            >
              {itemUsuarioRol.rol.rol}
            </MenuItem>
          ))}
        </Select>
      </Grid>,
  ...
  const obtenerUsuariosPeticion = async () => {
    try {
      ...
      const respuesta = await sesionPeticion({
        url: `${Constantes.baseUrl01}/usuarios`,
        params: {
          pagina: pagina,
          limite: limite,
  ...
  const cambiarEstadoUsuarioPeticion = async (usuario: UsuarioCRUDType) => {
    try {
      //setLoading(true)
      const respuesta = await sesionPeticion({
        url: `${Constantes.baseUrl01}/usuarios/${usuario.id}/${
  ...
  const restablecerPassUsuarioPeticion = async (usuario: UsuarioCRUDType) => {
    try {
      setLoading(true)
      const respuesta = await sesionPeticion({
        url: `${Constantes.baseUrl01}/usuarios/${usuario.id}/restauracion`,
        method: 'patch',
      })
  ...
  const reenviarCorreoActivacionPeticion = async (usuario: UsuarioCRUDType) => {
    try {
      setLoading(true)
      const respuesta = await sesionPeticion({
        url: `${Constantes.baseUrl01}/usuarios/${usuario.id}/reenviar`,
        method: 'patch',
      })
