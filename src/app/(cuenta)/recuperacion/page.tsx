...
export default function RecuperacionPage() {
  ...
  const recuperarPeticion = async ({ correo }: correoRecuperarType) => {
    try {
      ...
      const respuesta = await Servicios.post({
        url: `${Constantes.baseUrl01}/usuarios/recuperar`,
        body: {
  ...
  const validarRecuperarPeticion = async ({ codigo }: codigoRecuperarType) => {
    try {
      ...
      const respuesta = await Servicios.post({
        url: `${Constantes.baseUrl01}/usuarios/validar-recuperar`,
        body: {
          codigo: codigo,
        },
      })
                ...
                <form onSubmit={handleSubmit(recuperarPeticion)}>
                  <Box display={'flex'} flexDirection={'column'}>
                    ...
                    <Typography variant="body2" color="text.secondary">
                      Introduzca la dirección de correo electrónico asociado a
                      su cuenta y le enviaremos un enlace para restablecer su
                      contraseña.
                    </Typography>
                    <Box height={'20px'} />
                    <FormInputText
